...
function handleResizeRight(e: MouseEvent) {
	...
	propertiesPanelWidth.value = Math.min(
		Math.max(newWidth, minPanelWidth),
		maxPanelWidth,
	);
}
...
function handleResizeLeft(e: MouseEvent) {
	...
	stepsPanelWidth.value = Math.min(
		Math.max(newWidth, minStepsPanelWidth),
		maxStepsPanelWidth,
	);
}
...
const selectedStep = computed(() => {
	if (
		...
		selectedStepIndex.value >= formData.value.steps.length
	) {
		return null;
	}
...
const stepsWithNames = computed(() => {
	return formData.value.steps.filter(
		(s) => s.step_name && s.step_name.trim() !== "",
	);
});
...
const unassignedVariables = computed(() => {
	return availableVariables.value.filter(
		(v) => !assignedVariables.value.has(v.key),
	);
});
...
const messageTypes = [
	...
	},
	{
		...
		description: "Text with button options",
	},
	{
		value: "api_fetch",
		label: "API",
	...
	},
	{
		...
		label: "WA Flow",
		icon: MessageCircle,
		description: "WhatsApp Flow form",
	},
	{
		value: "transfer",
		label: "Transfer",
...
async function fetchWhatsAppFlows() {
	try {
		...
		whatsappFlows.value = allFlows.filter(
			(f: WhatsAppFlow) =>
				f.meta_flow_id && f.status?.toUpperCase() === "PUBLISHED",
		);
	} catch (error) {
...
async function fetchChatbotFlows() {
	try {
		const response = await chatbotService.listFlows();
		const data = response.data.data || response.data;
		// Filter out current flow to prevent self-reference loops (optional, but good UX)
		chatbotFlows.value = (data.flows || []).filter(
			(f: any) => !flowId.value || f.id !== flowId.value,
		);
	} catch (error) {
...
async function loadFlow(id: string) {
	...
	try {
		...
		formData.value = {
			...
			on_complete_action:
				flow.on_complete_action || flow.OnCompleteAction || "none",
			completion_config: {
			...
			panel_config: {
				sections:
					(flow.panel_config || flow.PanelConfig || {}).sections ||
					[],
			},
			enabled: flow.is_enabled ?? flow.IsEnabled ?? flow.enabled ?? true,
			steps: (flow.steps || flow.Steps || []).map(
				(s: any, idx: number) => ({
					id: s.id || s.ID,
					...
					api_config: {
						...
						headers:
							(s.api_config || s.ApiConfig || {}).headers || {},
						response_mapping:
							(s.api_config || s.ApiConfig || {})
								.response_mapping || {},
					},
					...
					validation_regex:
						s.validation_regex || s.ValidationRegex || "",
					validation_error:
						s.validation_error ||
						s.ValidationError ||
					...
					conditional_next:
						s.conditional_next || s.ConditionalNext || {},
					retry_on_invalid:
...
function setInputType(
	type: string | number | bigint | Record<string, any> | null,
) {
...
function setButtonNextStep(
	buttonId: string,
	targetStep: string | number | bigint | Record<string, any> | null,
) {
...
function addHeader() {
	if (!selectedStep.value) return;
	const headerNum =
		Object.keys(selectedStep.value.api_config.headers).length + 1;
...
function addResponseMapping() {
	if (!selectedStep.value) return;
	const mappingNum =
		Object.keys(selectedStep.value.api_config.response_mapping).length + 1;
...
function addCompletionHeader() {
	const headerNum =
		Object.keys(formData.value.completion_config.headers).length + 1;
...
function addFieldToSection(
	sectionIndex: number,
	variableKey: string | number | bigint | Record<string, any> | null,
) {
	if (typeof variableKey !== "string") return;
	const variable = availableVariables.value.find(
		(v) => v.key === variableKey,
	);
	if (!variable) return;
...
async function saveFlow() {
	...
	for (let i = 0; i < formData.value.steps.length; i++) {
		...
		if (step.message_type === "buttons" && step.buttons?.length > 0) {
			for (const btn of step.buttons) {
				...
				if (!btn.title?.trim()) {
					toast.error(
						`Step "${step.step_name || `Step ${i + 1}`}" has a button without a title. Button titles are required for WhatsApp.`,
					);
					selectStep(i);
					return;
				}
				...
				if (btn.type === "url") {
					if (!btn.url?.trim()) {
						toast.error(
							`Step "${step.step_name || `Step ${i + 1}`}" has a URL button "${btn.title}" without a URL.`,
						);
						selectStep(i);
						return;
					}
					...
					try {
						...
						toast.error(
							`Step "${step.step_name || `Step ${i + 1}`}" has an invalid URL for button "${btn.title}".`,
						);
						selectStep(i);
						return;
					}
	...
	try {
		const data = {
			...
			completion_config:
				formData.value.on_complete_action === "webhook"
					? formData.value.completion_config
					: {},
	...
	<div class="flex flex-col h-full bg-muted/30">
		...
		<div v-else class="flex-1 flex overflow-hidden">
					...
					<div class="p-2">
						...
						>
							<template #item="{ element: step, index }">
								...
								>
									<GripVertical
										class="h-4 w-4 text-muted-foreground cursor-grab drag-handle flex-shrink-0"
									/>
									<div class="flex-1 min-w-0">
										<div class="flex items-center gap-2">
											<Badge
												variant="outline"
												class="font-mono text-xs px-1.5"
												>{{ index + 1 }}</Badge
											>
											<span
										...
										>
											...
											/>
											<span>{{
												getStepLabel(step.message_type)
											}}</span>
										</div>
									</div>
									<Button
			...
			>
				...
				<div
					class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1 h-8 rounded-full bg-border group-hover:bg-primary/40 transition-colors"
				></div>
			</div>
					...
					<div class="p-4 space-y-4">
						...
						<Collapsible v-model:open="webhookHeadersOpen">
							...
							<CollapsibleContent class="pt-3 space-y-3">
									...
									>
										...
										<SelectContent>
											<SelectItem value="none"
												>No action</SelectItem
											>
											<SelectItem value="webhook"
												>Send to API/Webhook</SelectItem
											>
										</SelectContent>
									</Select>
								</div>
								...
								<template
									v-if="
										formData.on_complete_action ===
										'webhook'
									"
								>
									<div
										class="space-y-3 p-3 border rounded-lg bg-muted/30"
									>
										...
										<div class="space-y-2">
											...
											<div
												v-for="(_value, key) in formData
													.completion_config.headers"
												:key="key"
												class="flex gap-1"
											>
												<Input
						...
						<Collapsible v-model:open="panelConfigOpen">
							...
							<CollapsibleContent class="pt-3 space-y-3">
								<p class="text-[10px] text-muted-foreground">
									Configure which fields to show in the
									contact info panel when viewing chat.
								</p>
								...
								<div
									v-else
									class="text-[10px] text-muted-foreground p-2 border rounded bg-muted/30"
								>
									No variables available. Add "Store Response
									As" to steps or configure Response Mapping
									in API steps.
								</div>
								...
								<div class="space-y-2">
									...
									<div
										v-if="
											formData.panel_config.sections
												.length === 0
										"
										class="text-[10px] text-muted-foreground p-2 border rounded bg-muted/30 text-center"
									>
										No sections configured. Click "Add
										Section" to start.
									</div>

									<div
										v-for="(section, sectionIdx) in formData
					...
					<div class="p-4 space-y-4">
						...
						<div class="space-y-3">
							<div class="space-y-1.5">
								...
								<Input
									v-model="selectedStep.step_name"
									placeholder="step_1"
									class="h-8"
								/>
							</div>
						...
						<Collapsible v-model:open="messagesOpen">
							...
							<CollapsibleContent class="pt-3 space-y-3">
								...
								<template
									v-if="
										selectedStep.message_type === 'text' ||
										selectedStep.message_type === 'buttons'
									"
								>
									<div class="space-y-1.5">
										<Label class="text-xs"
								...
								>
									<div class="space-y-3">
										...
										<div class="space-y-2">
											...
											>
												...
												<div v-else class="space-y-2">
													<Input
														v-model="btn.id"
														:placeholder="`Button ID (default: btn_${idx + 1})`"
														class="h-7 text-xs"
													/>
													<div
														class="flex items-center gap-2"
													>
														...
														>
															...
															>
																<SelectValue
																	placeholder="Next step (sequential)"
																/>
															</SelectTrigger>
															<SelectContent>
																<SelectItem
																...
																<div
																	v-if="
																		stepsWithNames.length >
																		0
																	"
																	class="p-1"
																>
																	<div
																		class="text-[10px] text-muted-foreground px-2 py-1 font-medium"
																	>
																		Steps
																	</div>
																	<SelectItem
																...
																>
																	...
																	<SelectItem
																		v-for="flow in chatbotFlows"
																		:key="`goto-flow-${flow.id}`"
										...
										<p
											class="text-[10px] text-muted-foreground"
										>
											Reply buttons send user's choice
											back. URL buttons open a link. Use
											"Go to" to branch to different
											steps.
										</p>
									</div>
								</template>
									...
									<div class="space-y-3">
										<div class="space-y-1.5">
											...
											>
												...
												<SelectContent>
													<SelectItem
														v-for="wf in whatsappFlows"
														:key="wf.id"
														:value="wf.meta_flow_id"
													>
														{{ wf.name }}
													</SelectItem>
												</SelectContent>
								...
								<template
									v-if="
										selectedStep.message_type === 'transfer'
									"
								>
									<div class="space-y-3">
										<div class="space-y-1.5">
						...
						<Separator
							v-if="selectedStep.message_type !== 'transfer'"
						/>
						...
						>
							...
							<CollapsibleContent class="pt-3 space-y-3">
								...
								<div
									v-if="selectedStep.input_type === 'select'"
									class="space-y-1.5"
								>
									<Label class="text-xs"
						...
						>
							...
							<CollapsibleContent class="pt-3 space-y-3">
								<div class="space-y-1.5">
									...
									>
									<Input
										v-model="selectedStep.validation_regex"
										placeholder="^[A-Za-z ]+$"
								...
								<div class="space-y-1.5">
									...
									<Input
										v-model="selectedStep.validation_error"
										class="h-8 text-xs"
									/>
								</div>
								<div class="flex items-center gap-2">
									<Switch
										...
										@update:checked="
											selectedStep.retry_on_invalid =
												$event
										"
									/>
									<Label class="text-xs"
										>Retry on invalid</Label
									>
									<Input
										v-if="selectedStep.retry_on_invalid"
										v-model.number="
											selectedStep.max_retries
										"
										type="number"
										min="1"
										max="10"
						...
						>
							...
							<CollapsibleContent class="pt-3 space-y-3">
								<div class="space-y-1.5">
									...
									>
									<Input
										v-model="selectedStep.skip_condition"
										placeholder="phone != ''"
										class="h-8 text-xs font-mono"
									/>
									<p class="text-xs text-muted-foreground">
		...
		<AlertDialog v-model:open="deleteStepDialogOpen">
			<AlertDialogContent>
				...
				<AlertDialogFooter>
					...
					<AlertDialogAction @click="deleteStep"
