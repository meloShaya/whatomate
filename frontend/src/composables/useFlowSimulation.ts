...
export function useFlowSimulation(
	...
	const { evaluateCondition, interpolateVariables, validateInput } =
	...
	const currentStep = computed(() => {
		if (
			...
			state.currentStepIndex >= steps.value.length
		) {
			return null;
		}
	...
	const expectedInputType = computed(() => {
		...
		if (currentStep.value.input_type !== "none")
			return currentStep.value.input_type;
		return null;
	});
	...
	async function processStep(step: FlowStep): Promise<void> {
		...
		// Simulate typing delay (matches backend 3s delay, but faster for UI preview)
		if (
			step.message_type !== "api_fetch" &&
			step.message_type !== "script"
		) {
			state.status = "typing";
			await delay(1500);
			state.status = "running";
		...
		if (step.message_type === "transfer") {
			const teamName =
				step.transfer_config.team_id === "_general"
					? "General Queue"
	...
	async function processApiStep(step: FlowStep): Promise<string> {
		...
		addMessage(
			"system",
			`Calling API: ${step.api_config.method} ${step.api_config.url}`,
		);

		const result = await apiMocker.executeMockedApiCall(
			step,
			state.variables,
		);
		...
		if (result.success && result.data) {
			...
			addMessage(
				"debug",
				`API Response (${result.duration}ms): ${JSON.stringify(result.data)}`,
			);

			// Use message template with extracted variables
			return interpolateVariables(step.message, {
				...state.variables,
				...extracted,
			});
		} else {
			...
			return (
				step.api_config.fallback_message ||
				step.message ||
	...
	async function processUserInput(input: UserInput): Promise<void> {
		...
		if (typeof input === "string") {
			...
			if (
				step.validation_regex &&
				!validateInput(input, step.validation_regex)
			) {
	...
	async function processWhatsAppFlowCompletion(
	...
	function handleValidationError(step: FlowStep): void {
		...
		if (
			step.retry_on_invalid &&
			state.currentRetryCount < step.max_retries
		) {
			addMessage(
				"bot",
				step.validation_error || "Invalid input. Please try again.",
				{
			...
			addMessage(
				"system",
				"Maximum retries reached. Moving to next step.",
			);
			state.currentRetryCount = 0;
			moveToNextStep(step);
		}
	...
	async function moveToNextStep(
		...
		if (
			buttonId &&
			currentStepObj.conditional_next &&
			currentStepObj.conditional_next[buttonId]
		) {
			nextStepName = currentStepObj.conditional_next[buttonId];
			log("branch", currentStepObj.step_name, {
				type: "conditional",
				buttonId,
		...
		if (nextStepName) {
			// Check for flow transfer
			if (nextStepName.startsWith("flow:")) {
				const flowName = nextStepName.substring(5);
				log("flow_transfer", currentStepObj.step_name, {
					targetFlow: flowName,
				});
				addMessage("system", `⟳ Transferred to Flow: ${flowName}`);
				state.status = "completed";
				return;
			}
	...
	function completeFlow(): void {
		...
		log("flow_complete", state.currentStepName || undefined, {
			reason: "end",
		});
		state.status = "completed";
	}
	...
	function resumeSimulation(): void {
		if (state.status === "paused") {
			if (currentStep.value) {
				const needsInput =
					currentStep.value.input_type !== "none" ||
