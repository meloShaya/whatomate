...
watch(
	...
	async () => {
		...
		if (chatScrollRef.value?.$el) {
			const scrollArea =
				chatScrollRef.value.$el.querySelector(
...
const lastButtonMessage = computed(() => {
	...
	const lastBotMessage = [...state.messages]
		.reverse()
		.find((m) => m.type === "bot" && m.buttons?.length);
...
const currentApiStep = computed(() => {
	if (apiMocker.currentMockStep.value) {
		return props.steps.find(
			(s) => s.step_name === apiMocker.currentMockStep.value,
		);
	}
	return null;
});
...
<template>
	<div class="flex-1 flex h-full">
		...
		<div class="flex-1 flex flex-col min-w-0">
			...
			<div
				class="flex-1 flex items-center justify-center p-4 bg-[#efeae2] dark:bg-[#0b141a]"
			>
				<div
					id="preview-phone-frame"
					class="w-full max-w-sm bg-[#efeae2] dark:bg-[#0b141a] rounded-2xl overflow-hidden shadow-xl flex flex-col h-[600px] relative"
				>
					<!-- Chat Header -->
					<div
						class="bg-[#075e54] dark:bg-[#202c33] text-white px-4 py-3 flex items-center gap-3 flex-shrink-0"
					>
						<div
					...
					<ScrollArea ref="chatScrollRef" class="flex-1 p-4">
						<div class="space-y-3">
							...
							>
								<MessageSquare
									class="h-12 w-12 mx-auto text-gray-300 dark:text-gray-600 mb-4"
								/>
								<p
							...
							<!-- Typing Indicator -->
							<div
								v-if="state.status === 'typing'"
								class="flex justify-start"
							>
								<div class="max-w-[85%]">
									<div
										class="bg-white dark:bg-[#202c33] rounded-lg rounded-tl-none shadow-sm p-4"
									>
										<div
											class="flex gap-1.5 items-center h-2"
										>
											<span
							...
							>
								<div class="max-w-[85%]">
									<PreviewButtonGroup
										v-if="
											lastButtonMessage.buttons.length <=
											3
										"
					...
					<PreviewInputBar
						...
						:disabled="
							!isWaitingForInput || expectedInputType === 'button'
						"
		...
		<ApiMockDialog
			...
			@update:open="
				(open) => {
					if (!open) apiMocker.submitMockConfig(null);
				}
			"
