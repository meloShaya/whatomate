...
function getButtonDestination(
	...
	if (targetStepName) {
		// Check for Flow Jump
		if (targetStepName.startsWith("flow:")) {
			const flowName = targetStepName.substring(5);
			return {
				targetIdx: -2,
				targetName: `Flow: ${flowName}`,
				isFlowJump: true,
			}; // -2 = Flow Jump
		}

		const targetIdx = props.steps.findIndex(
			(s) => s.step_name === targetStepName,
		);
		if (targetIdx !== -1) {
	...
	if (nextIdx < props.steps.length) {
		return {
			targetIdx: nextIdx,
			targetName: props.steps[nextIdx].step_name || `Step ${nextIdx + 1}`,
		};
	}
	...
	while (queue.length > 0) {
		...
		if (hasButtons(step)) {
			...
			buttons.forEach((btn, btnIdx) => {
				const dest = getButtonDestination(
					step,
					currentIdx,
					btn,
					btnIdx,
	...
	for (const stepIdx of reachableSteps.value) {
		...
		if (hasButtons(step)) {
			...
			for (let btnIdx = 0; btnIdx < buttons.length; btnIdx++) {
				...
				if (dest.targetIdx === -1 || dest.targetIdx === -2) {
					return true; // This button goes to END or another FLOW
				}
			}
		} else {
...
function updateConnections() {
	...
	svgSize.value = {
		width: Math.max(1200, contentWidth + 200),
		height: Math.max(2000, contentHeight),
	};
	...
	const getPos = (el: HTMLElement) => {
		...
		return {
			...
			centerX:
				rect.left + rect.width / 2 - containerRect.left + scrollLeft,
	...
	props.steps.forEach((step, stepIdx) => {
		...
		if (!hasButtons(step)) {
			...
			if (
				stepEl &&
				(nextStepEl || (stepIdx === props.steps.length - 1 && endEl))
			) {
			...
			replyButtons.forEach((btn, btnIdx) => {
				...
				const targetEl =
					dest.targetIdx >= 0
						? stepRefs.value.get(dest.targetIdx)
						: endRef.value;
				...
				if (isJump && dest.targetIdx >= 0) {
					...
					if (!targetCounts[dest.targetIdx])
						targetCounts[dest.targetIdx] = 0;
...
watch(
	() => [props.steps, props.selectedStepIndex],
	() => {
...
<template>
	<div
		class="h-full flex flex-col bg-[#fafafa] dark:bg-[#0a0a0a] overflow-hidden"
	>
		<!-- Header -->
		<div
			class="px-4 py-3 border-b bg-white dark:bg-[#111] flex items-center justify-between flex-shrink-0"
		>
		...
		>
			...
			>
				...
				<g v-for="(conn, idx) in connections" :key="idx">
					...
					<path
						v-if="
							conn.color !== '#9ca3af' && conn.color !== '#22c55e'
						"
						:d="conn.path"
						fill="none"
						stroke="white"
					...
					<path
						...
						:marker-end="
							conn.color === '#22c55e'
								...
								: conn.color === '#9ca3af'
									? 'url(#arrow-gray)'
									: `url(#arrow-${lineColors.indexOf(conn.color)})`
						"
					/>
			...
			<!-- Flow Nodes -->
			<div
				class="relative p-8 flex flex-col items-center min-h-full"
				style="z-index: 1"
			>
				<!-- Start Node -->
				<div
				...
				>
					<span class="mb-2 text-sm font-bold text-green-600"
						>START</span
					>
					<div
						class="w-16 h-16 rounded-full bg-green-500 flex items-center justify-center shadow-lg group-hover:ring-4 group-hover:ring-green-200 transition-all"
					>
				...
				<template v-for="(step, idx) in steps" :key="`step-${idx}`">
					...
					>
						...
						<div
							:class="[
								'flex items-center gap-3 p-3 rounded-t-xl',
								isUnreachable(idx)
						...
						>
							...
							<span
								...
								>{{ step.step_name || `Step ${idx + 1}` }}</span
							>
							<Badge
								:variant="
									isUnreachable(idx)
						...
						<div v-if="hasButtons(step)" class="px-3 pb-3">
							...
							>
								...
								<span
									>{{ getReplyButtons(step).length }} button{{
										getReplyButtons(step).length > 1
											? "s"
											: ""
									}}
									- routes to specific steps</span
								>
							</div>
						</div>
					</div>
					...
					<div
						v-if="hasButtons(step)"
						class="flex gap-3 mb-8 flex-wrap justify-center"
					>
						<div
							...
							:class="[
								...
								getButtonDestination(step, idx, btn, btnIdx)
									.targetIdx !==
								idx + 1
				...
				>
					...
					<p
						v-if="!isEndReachable && steps.length > 0"
						class="text-xs text-red-600 mt-1 text-center max-w-[200px]"
					>
						Flow has no exit path - will loop forever
					</p>
					<p
						v-else-if="stepsInLoop.size > 0 && steps.length > 0"
						class="text-xs text-amber-600 mt-1 text-center max-w-[200px]"
					>
