...
<template>
	<div class="flex h-screen bg-[#0a0a0b] light:bg-gray-50">
		...
		<!-- Mobile header -->
		<header
			class="fixed top-0 left-0 right-0 z-50 flex h-12 items-center justify-between border-b border-white/[0.08] light:border-gray-200 bg-[#0a0a0b]/95 light:bg-white/95 backdrop-blur-sm px-3 md:hidden"
		>
			<RouterLink to="/" class="flex items-center gap-2">
				<div
					class="h-7 w-7 rounded-lg bg-gradient-to-br from-emerald-500 to-green-600 flex items-center justify-center shadow-lg shadow-emerald-500/20"
				>
		...
		>
			...
			<div
				class="hidden md:flex h-12 items-center justify-between px-3 border-b border-white/[0.08] light:border-gray-200"
			>
				<RouterLink to="/" class="flex items-center gap-2">
					<div
						class="h-7 w-7 rounded-lg bg-gradient-to-br from-emerald-500 to-green-600 flex items-center justify-center shadow-lg shadow-emerald-500/20"
					...
					<template v-for="item in navigation" :key="item.path">
						...
						>
							<component
								...
								aria-hidden="true"
							/>
							<span :class="isCollapsed && 'md:sr-only'">{{
								$t(item.name)
							}}</span>
						</RouterLink>
						...
						<!-- Submenu items -->
						<template
							v-if="item.children && item.active && !isCollapsed"
						>
							<RouterLink
		...
		<main
			id="main-content"
			class="flex-1 overflow-hidden pt-12 md:pt-0 bg-[#0a0a0b] light:bg-gray-50"
			role="main"
		>
			<RouterView />
		</main>
